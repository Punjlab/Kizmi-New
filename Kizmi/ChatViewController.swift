...
class ChatViewController: QMChatViewController, QMChatServiceDelegate, UIActionSheetDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, QMChatAttachmentServiceDelegate, QMChatConnectionDelegate, QMChatCellDelegate, QMDeferredQueueManagerDelegate, QMPlaceHolderTextViewPasteDelegate {
    ...
    @IBOutlet var navigateITems: UINavigationItem!
    
    var reciepentUser : QBUUser!
    ...
    override func viewDidLoad() {
        ...
        QBRequest.user(withID: UInt(UInt(self.dialog!.recipientID)), successBlock: { (response, user) in
            print(response)
            
            self.reciepentUser = user
            print(self.reciepentUser)
            self.updateTitle()
        ...
        let rightBarButtonItem = UIBarButtonItem.init(image: UIImage(named: "info.png"), style: .done, target: self, action: #selector(self.goToUserDetail))
    ...
    func goToUserDetail() {
        ...
        WebHelper.requestGetUrl("\(GlobalConstant.BaseURL)get_profile?user_id=\(reciepentUser.externalUserID)", controllerView: self, success: {(_ response: [AnyHashable: Any]) -> Void in
            //Success
            ...
            if  responseDict.count == 0
            {
                ...
                if status == "1" {
                    ...
                    DispatchQueue.main.async {
                        ...
                        child.dictProfile = responseDict["result"] as! NSDictionary
                        child.strFrom = "Chat"
                        child.strStrUserID = self.dialog.id
    ...
    func updateTitle() {
        ...
        if self.dialog.type != QBChatDialogType.private {
            ...
            self.title = self.dialog.name
                ...
                self.title = self.reciepentUser.fullName
    ...
    func loadMessages() {
        ...
        ServicesManager.instance().chatService.messages(withChatDialogID: currentDialogID, completion: {
            ...
            guard response.error == nil else {
                GlobalConstant.showAlertMessage(withOkButtonAndTitle: GlobalConstant.AppName, andMessage: (response.error?.error?.localizedDescription)!, on: self!)
                return
            }
    ...
    func sendMessage(message: QBChatMessage) {
        ...
        ServicesManager.instance().chatService.send(message, toDialogID: self.dialog.id!, saveToHistory: true, saveToStorage: true) { (error) ->
            ...
            if error != nil {
                ...
                QBRequest.sendPush(withText :"\(String(describing:  QBSession.current.currentUser!.fullName!)) send you a new message", toUsers: "\(self.dialog.recipientID)", successBlock: { (response, event) in
        ...
        if message.readIDs != nil {
            ...
            if !messageReadIDs.isEmpty {
                for readID in messageReadIDs {
                    ...
                    readLogins.append(unwrappedUser.email!)
                }
        ...
        if message.deliveredIDs != nil {
            ...
            if !messageDeliveredIDs.isEmpty {
                for deliveredID in messageDeliveredIDs {
                    ...
                    if readLogins.contains(unwrappedUser.email!) {
                        continue
                    }
                    
                    deliveredLogins.append(unwrappedUser.email!)
    ...
    override func topLabelAttributedString(forItem messageItem: QBChatMessage!) -> NSAttributedString? {
        ...
        if let topLabelText = ServicesManager.instance().usersService.usersMemoryStorage.user(withID: messageItem.senderID)?.email {
    ...
    override func collectionView(_ collectionView: QMChatCollectionView!, configureCell cell: UICollectionViewCell!, for indexPath: IndexPath!) {
        ...
        if let attachmentCell = cell as? QMChatAttachmentCell {
            ...
            if attachmentCell is QMChatAttachmentIncomingCell {
                ...
                chatCell.containerView?.bgColor = UIColor(red: 18.0/255.0, green: 73.0/255.0, blue: 96.0/255.0, alpha: 1.0)
            }
    ...
    func chatCell(_ cell: QMChatCell!, didTapOn result: NSTextCheckingResult) {
        ...
        case NSTextCheckingResult.CheckingType.phoneNumber:
            ...
            if !self.canMakeACall() {
                ...
                GlobalConstant.showAlertMessage(withOkButtonAndTitle: GlobalConstant.AppName, andMessage: "Your Device can't make a phone call", on: self)
                break
            }
