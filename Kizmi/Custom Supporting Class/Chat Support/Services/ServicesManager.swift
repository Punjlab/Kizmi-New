...
class ServicesManager: QMServicesManager {
    ...
    func downloadCurrentEnvironmentUsers(successBlock:(([QBUUser]?) -> Void)?, errorBlock:((NSError) -> Void)?) {
        ...
        self.usersService.searchUsers(withTags: ["dev"]).continueWith(block: { [weak self] (task) -> Any? in
    ...
    func sortedUsers() -> [QBUUser]? {
        ...
        let sortedUsers = unsortedUsers.sorted(by: { (user1, user2) -> Bool in
            return user1.email!.compare(user2.email!, options:NSString.CompareOptions.numeric) == ComparisonResult.orderedAscending
        })
    ...
    /**
     ...
     - returns: [QBUUser]
     */
    func sortedUsersWithoutCurrentUser(externalUserID : UInt) -> [QBUUser]? {
        ...
        let sortedUsersWithoutOthertUser = sortedUsersWithoutCurrentUser.filter({ $0.externalUserID == externalUserID})
        
        return sortedUsersWithoutOthertUser
    }
